...
declare -a arr=("Aedes_aegypti" "Anopheles_albimanus" "Anopheles_arabiensis" "Anopheles_gambiae_str._PEST" "Anopheles_sinensis" "Anopheles_stephensi")

## loop through array
for i in "${arr[@]}"
do
   echo "$i"
   sed "s/basetax/$i/g" /work/soghigian_lab/abdullah.zubair2/507/Orthograph/orthograph.conf > /work/soghigian_lab/abdullah.zubair2/507/Final_OrthoSet/orthoset_construction/culicidae_orthologs/configs/orthograph_$i.conf
   orthograph-analyzer --configfile configs/orthograph_$i.conf --input-is-amino-acid
   orthograph-reporter --configfile configs/orthograph_$i.conf 
   perl /work/soghigian_lab/apps/Orthograph-0.7.1/summarize_orthograph_results.pl -i output -o done -c -t -s -u -d /work/soghigian_lab/abdullah.zubair2/507/Orthograph/ref_tax.txt
